...
tests <- list(list(data_gen = get_data, test_rounds = 10))

tests <- mapply(function(t, block_size) { c(t, block_size = block_size) },
                tests, rep(c(2, 4), each = length(tests)),
                SIMPLIFY = FALSE, USE.NAMES = FALSE)
tests <- mapply(function(t, algorithm) { c(t, algorithm = algorithm) },
                tests, rep(c("directed", "undirected", "paper"), each = length(tests)),
                SIMPLIFY = FALSE, USE.NAMES = FALSE)
...
for (test in tests) {
  ...
  for (r in 1:test$test_rounds) {
    data <- test$data_gen()
    blocking <- get_blocking(data, test$block_size, test$algorithm, test$MIS_method, test$treetype)
    Rrep_blocking <- Rrep_get_blocking(data, test$block_size, test$algorithm, test$MIS_method, test$treetype)
